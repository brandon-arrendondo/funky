...
| `assert( cond )` / `testcase( expr )` inner-paren space stripped | `space_inside_parens` was `bool` defaulting to `false` (remove). Changed to `SpaceOption` tristate defaulting to `"preserve"`, which passes source spacing through unchanged — matching uncrustify's passive behaviour. | `config.rs`, `formatter.rs` |
| `space_inside_brackets` also normalized by default | Same as above — changed from `bool` `false` to `SpaceOption` `"preserve"`. | `config.rs`, `formatter.rs` |
